...
public class StdTestObjectTypes {
    ...
    // TEST
    private static EID TestTEST = EidFactory.getDefault().createAndPreserveStr("88311f83-818c-46ed-8a9a-cec4f3707365");
    ...
    public final static EidMap<TestObjectTypeDto> types = new DefaultEidMap<>(
            Collections.unmodifiableMap(new LinkedHashMap<EID, TestObjectTypeDto>() {
                ...
                {
                    API_FEATURES_TOT.setLabel("API Features");
                    API_FEATURES_TOT.setId(API_FEATURES_ID);
                    API_FEATURES_TOT.setParent(WEB_SERVICE_TOT);
                    API_FEATURES_TOT.setDescription("API Features service.");
                    API_FEATURES_TOT.setDetectionExpression("boolean(/child::*[(local-name() = 'API_FEATURES')])",
                            ExpressionType.XPATH);
                    API_FEATURES_TOT.setLabelExpression(owsLabelExpression, ExpressionType.XPATH);
                    API_FEATURES_TOT.setDescriptionExpression(owsDescriptionExpression, ExpressionType.XPATH);
                    API_FEATURES_TOT
                            .setDefaultPathAndQuery("");
                    API_FEATURES_TOT.setUriDetectionExpression("(service=wfs.*(version=2\\.0\\.|acceptversions=2\\.0\\.))|"
                ...
                }
                {
                    ...
                    METADATA_RECORDS_TOT.setDetectionExpression(
                            "boolean(/*["
                                    + "(local-name() = 'GetRecordsResponse' and starts-with(namespace-uri(), 'http://www.opengis.net/cat/csw/') and (count(//*[local-name() = 'MD_Metadata'])>0)) or "
